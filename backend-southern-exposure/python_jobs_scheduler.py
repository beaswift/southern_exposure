...
def threaded_function_start(name_var):
  thisJob = schedule.every(5).seconds.do(job, name = name_var)
  ...
  while True:
    ...
    if name_var not in accepted_threads:
      ...
      pass
      #thisJob = schedule.every(1).seconds.do(job)
...
def connect_db():
  #global accepted_threads
  #global threads
  threading.Timer(8.0, connect_db).start()
  try:
    ...
    if sorted(jobs_on_table) != sorted(accepted_threads):
      ...
      for each  in list(set(jobs_on_table) - set(accepted_threads)):
        ...
        threads.append(Thread(target = threaded_function_start(each)))
